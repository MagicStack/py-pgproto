...
cdef inline _decode_points(frb.Buffer *buf):
    cdef:
        int32_t npts = hton.unpack_int32(frb.read(buf, 4))
    ...
    for i in range(npts):
        x = hton.unpack_double(frb.read(buf, 8))
        y = hton.unpack_double(frb.read(buf, 8))
        point = Point(x, y)
...
cdef box_decode(CodecContext settings, frb.Buffer *buf):
    cdef:
        double high_x = hton.unpack_double(frb.read(buf, 8))
        double high_y = hton.unpack_double(frb.read(buf, 8))
        double low_x = hton.unpack_double(frb.read(buf, 8))
        double low_y = hton.unpack_double(frb.read(buf, 8))
...
cdef line_decode(CodecContext settings, frb.Buffer *buf):
    cdef:
        double A = hton.unpack_double(frb.read(buf, 8))
        double B = hton.unpack_double(frb.read(buf, 8))
        double C = hton.unpack_double(frb.read(buf, 8))
...
cdef lseg_decode(CodecContext settings, frb.Buffer *buf):
    cdef:
        double p1_x = hton.unpack_double(frb.read(buf, 8))
        double p1_y = hton.unpack_double(frb.read(buf, 8))
        double p2_x = hton.unpack_double(frb.read(buf, 8))
        double p2_y = hton.unpack_double(frb.read(buf, 8))
...
cdef point_decode(CodecContext settings, frb.Buffer *buf):
    cdef:
        double x = hton.unpack_double(frb.read(buf, 8))
        double y = hton.unpack_double(frb.read(buf, 8))
...
cdef path_decode(CodecContext settings, frb.Buffer *buf):
    cdef:
        int8_t is_closed = <int8_t>(frb.read(buf, 1)[0])
...
cdef circle_decode(CodecContext settings, frb.Buffer *buf):
    cdef:
        double center_x = hton.unpack_double(frb.read(buf, 8))
        double center_y = hton.unpack_double(frb.read(buf, 8))
        double radius = hton.unpack_double(frb.read(buf, 8))
