...
cdef bool_decode(CodecContext settings, frb.Buffer *buf):
    return frb.read(buf, 1)[0] is b'\x01'
...
cdef int2_decode(CodecContext settings, frb.Buffer *buf):
    return cpython.PyLong_FromLong(hton.unpack_int16(frb.read(buf, 2)))
...
cdef int4_decode(CodecContext settings, frb.Buffer *buf):
    return cpython.PyLong_FromLong(hton.unpack_int32(frb.read(buf, 4)))
...
cdef uint4_decode(CodecContext settings, frb.Buffer *buf):
    return cpython.PyLong_FromUnsignedLong(
        <uint32_t>hton.unpack_int32(frb.read(buf, 4)))
...
cdef int8_decode(CodecContext settings, frb.Buffer *buf):
    return cpython.PyLong_FromLongLong(hton.unpack_int64(frb.read(buf, 8)))
