...
with open(str(_ROOT / 'pgbase' / '__init__.py')) as f:
    for line in f:
        ...
        raise RuntimeError(
            'unable to read the version from pgbase/__init__.py')
...
class sdist(setuptools_sdist.sdist, VersionMixin):
    ...
    def make_release_tree(self, base_dir, files):
        ...
        self._fix_version(pathlib.Path(base_dir) / 'pgbase' / '__init__.py')
...
class build_py(setuptools_build_py.build_py, VersionMixin):
    ...
    def build_module(self, module, module_file, package):
        ...
        if module == '__init__' and package == 'pgbase':
...
class build_ext(distutils_build_ext.build_ext):
    ...
    def finalize_options(self):
        ...
        try:
            ...
            raise RuntimeError(
                'please install {} to compile pgbase from source'.format(
                    CYTHON_DEPENDENCY))

        cython_dep = pkg_resources.Requirement.parse(CYTHON_DEPENDENCY)
        if Cython.__version__ not in cython_dep:
            raise RuntimeError(
                'pgbase requires {}, got Cython=={}'.format(
...
setuptools.setup(
    name='pgbase',
    version=VERSION,
    description='Sans IO PosgtreSQL driver',
